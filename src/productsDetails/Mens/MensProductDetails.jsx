    ...
    <div className="flex justify-center items-center">
      <div className="h-[80vh] w-[90%] grid grid-cols-1 md:grid-cols-2 p-2 gap-1">
        ...
        <div className="h-full flex justify-around items-center flex-col p-2 bg-gray-100 rounded-2xl">
          ...
          <button onClick={(()=>{navigate("/MensSection")})} className="border-2 px-2 rounded-full md:text-xl cursor-pointer"> <i class="ri-arrow-left-fill"></i> Go Back</button>
        </div>
      </div>
    </div>
  );
...
export const mensProducDetalsLoder = async ({ params }) => {
  const { id } = params;
  const res = await fetch("http://localhost:3000/topMensCollection/" + id);
  return res.json();
};
